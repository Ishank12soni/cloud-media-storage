...
async function createShare(req, res) {
  try {
    ...
    const { data: existing, error: existingError } = await supabase.from("shares")
      .select("*")
    ...
    if (existing) {
      const { data, error } = await supabase.from("shares")
        ...
        .select().single();
      if (error) throw error;
      return res.json({ success: true, share: data, updated: true });
    }
    ...
    if (error) throw error;
    res.status(201).json({ success: true, share: data, updated: false });
  } catch (error) {
